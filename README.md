# SIMON GAME.
The game is played by a single player and a computer.

### INITIALLY COMPUTER TURN
Initially we press any key from our PC to start the game.
The game starts with a blank sequence of colors.
The computer generates a random sequence of colors and displays it to the player.

### THE USER TURN
The player must then repeat the sequence back to the computer by clicking on the corresponding colors in the correct order.
If the player succeeds, the computer generates a new sequence by adding one more color to the previous sequence.
If the player fails, the game is over, we blinked a red backround to describe about the game is over.

### WHEN THE GAME IS OVER! WE CAN PRESS ANY KEY TO RESTART THE GAME
The game continues until the player fails to repeat the sequence correctly.
The goal of the game is to get the longest sequence possible.
and a score counter to display the current score.

### IT IS MADE BY THESE LANGUAGES
The game is played in a browser using HTML, CSS, and JavaScript.
